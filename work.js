...
// 生成域名键名
function getDomainKey(domainId) {
...
async function getDomain(id) {
  try {
    const domainStr = await DOMAIN_MONITOR.get(getDomainKey(id));
    if (!domainStr) {
      return null;
    }
...
async function getAllDomains() {
  try {
    // 先获取索引
    const indexStr = await DOMAIN_MONITOR.get('domains_index') || '[]';
    const domainIds = JSON.parse(indexStr);
    
    // 并行获取所有域名
    const domainPromises = domainIds.map(id => getDomain(id));
...
// 获取多个域名
async function getMultipleDomains(ids) {
  if (!ids || ids.length === 0) {
    return [];
  }
  ...
  try {
    // 并行获取多个域名
    const domainPromises = ids.map(id => getDomain(id));
    const domains = await Promise.all(domainPromises);
    
    // 过滤掉不存在的域名
    return domains.filter(domain => domain !== null);
  } catch (error) {
    console.error('获取多个域名失败:', error);
    return [];
  }
}

// 更新域名索引
async function updateDomainIndex(ids) {
  try {
    await DOMAIN_MONITOR.put('domains_index', JSON.stringify(ids));
    return true;
  } catch (error) {
    console.error('更新域名索引失败:', error);
    return false;
  }
}

// 从索引中移除域名
async function removeFromDomainIndex(id) {
  try {
    const indexStr = await DOMAIN_MONITOR.get('domains_index') || '[]';
    let domainIds = JSON.parse(indexStr);
    
    // 过滤掉要删除的ID
    domainIds = domainIds.filter(domainId => domainId !== id);
    
    // 更新索引
    await updateDomainIndex(domainIds);
    return true;
  } catch (error) {
    console.error('从索引中移除域名失败:', error);
    return false;
  }
}
...
async function addDomain(domainData) {
  try {
    ...
    const domain = {
      ...
      customNote: domainData.customNote || '',
      noteColor: domainData.noteColor || '',
      renewLink: domainData.renewLink || '',
      renewCycle: domainData.renewCycle || {
        value: 1,
        unit: 'year'
      },
      price: domainData.price || '',
      lastRenewed: domainData.lastRenewed || null,
    ...
    // 保存域名
    await DOMAIN_MONITOR.put(getDomainKey(id), JSON.stringify(domain));
...
async function updateDomain(id, domainData) {
  try {
    // 检查域名是否存在
    const existingDomain = await getDomain(id);
    if (!existingDomain) {
    ...
    const updatedDomain = {
      ...
      registrationDate: domainData.registrationDate,
      expiryDate: domainData.expiryDate,
      registrar: domainData.registrar !== undefined ? domainData.registrar : existingDomain.registrar,
      customNote: domainData.customNote !== undefined ? domainData.customNote : existingDomain.customNote,
      noteColor: domainData.noteColor !== undefined ? domainData.noteColor : existingDomain.noteColor,
      renewLink: domainData.renewLink !== undefined ? domainData.renewLink : existingDomain.renewLink,
      renewCycle: domainData.renewCycle || existingDomain.renewCycle,
      price: domainData.price !== undefined ? domainData.price : existingDomain.price,
      lastRenewed: domainData.lastRenewed !== undefined ? domainData.lastRenewed : existingDomain.lastRenewed,
      updatedAt: new Date().toISOString(),
      notifySettings: domainData.notifySettings !== undefined ? domainData.notifySettings : existingDomain.notifySettings
    };
    
    // 保存更新后的域名
    await DOMAIN_MONITOR.put(getDomainKey(id), JSON.stringify(updatedDomain));
...
async function deleteDomain(id) {
  try {
    ...
    // 删除域名数据
    await DOMAIN_MONITOR.delete(getDomainKey(id));
    
    // 从索引中移除
    await removeFromDomainIndex(id);
...
async function renewDomain(id, renewData) {
  try {
    ...
    if (new Date(existingDomain.expiryDate) < now) {
      renewedDomain.renewedFromExpired = true;
      renewedDomain.renewStartDate = now.toISOString();
    }
    
    // 如果没有设置续期周期，则设置默认值
    if (!renewedDomain.renewCycle) {
      renewedDomain.renewCycle = {
    ...
    // 保存更新后的域名
    await DOMAIN_MONITOR.put(getDomainKey(id), JSON.stringify(renewedDomain));
    
    return renewedDomain;
  } catch (error) {
    console.error('域名续期失败:', error);
    throw error;
  }
}
...
async function getTelegramConfig() {
  try {
    ...
    return {
      ...
      botToken: tokenFromEnv || tokenFromCode ? '' : (config.botToken || ''),
      chatIdFromEnv: chatIdFromEnv || chatIdFromCode,
...
async function getTelegramConfigWithToken() {
  try {
    ...
    console.error('获取完整Telegram配置失败:', error);
    return {
      enabled: false,
      botToken: '',
      chatId: '',
...
async function getBarkConfigWithKey() {
  try {
    ...
    console.error('获取完整Bark配置失败:', error);
    return {
      enabled: false,
      barkKey: '',
...
async function sendBarkMessage(config, message, title) {
  try {
    ...
    console.error('发送Bark消息失败:', error);
    throw error;
  }
}
...
async function testSingleDomainNotification(id) {
  try {
    ...
    console.error('测试单个域名通知失败:', error);
    throw error;
  }
}
// 发送域名过期通知
async function sendDomainExpiryNotification(domain) {
  try {
    // 获取通知配置
    const tgConfig = await getTelegramConfigWithToken();
    const barkConfig = await getBarkConfigWithKey();
    
    let notificationSent = false;
    
    // 构建通知消息
    const message = buildNotificationMessage(domain);
    
    // 发送Telegram通知
    if (tgConfig.enabled && tgConfig.botToken && tgConfig.chatId) {
      await sendTelegramMessage(tgConfig, message);
      notificationSent = true;
    }
    
    // 发送Bark通知
    if (barkConfig.enabled && barkConfig.barkKey) {
      await sendBarkMessage(barkConfig, message, '域名过期提醒');
      notificationSent = true;
    }
...
async function checkDomainExpiry() {
  try {
    ...
    if (!domains.length) {
      console.log('没有域名需要检查');
      return;
    }
    
    const now = new Date();
    const notificationThreshold = 30; // 30天内过期的域名需要通知
    ...
    for (const domain of domains) {
      ...
      // 检查是否需要发送通知
      if (daysUntilExpiry <= notificationThreshold && daysUntilExpiry >= 0) {
        await sendDomainExpiryNotification(domain);
      }
    }
  } catch (error) {
    console.error('检查域名过期状态失败:', error);
  }
}

// 构建通知消息
function buildNotificationMessage(domain) {
  const now = new Date();
  const expiryDate = new Date(domain.expiryDate);
  const daysUntilExpiry = Math.ceil((expiryDate - now) / (1000 * 60 * 60 * 24));
  ...
  let message = `域名过期提醒 ${status}\n`;
  message += `域名: <b>${domain.name}</b>\n`;
  
  if (domain.customNote) {
    message += `备注: ${domain.customNote}\n`;
  }
  
  message += `到期日期: ${formatDate(domain.expiryDate)}\n`;
  message += `剩余天数: ${daysUntilExpiry} 天\n`;
...
async function checkExpiringDomains() {
  try {
    ...
    console.error('检查即将到期域名失败:', error);
  }
}
